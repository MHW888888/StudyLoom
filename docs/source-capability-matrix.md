...
| v1.1 | Office source fidelity | Real DOCX/PPTX OpenXML extraction with warnings for complex structure. |
| v1.2 | Wiki and mindmap | Optional source-grounded wiki, Markmap, Mermaid, and JSON graph exports. |
| v1.3 | Browser extension hardening | Sanitized current-page export, redaction, schema metadata, and no cookie/history/storage permissions. |
| v1.4 | Optional local OCR/ASR/keyframes | Sidecar/Tesseract OCR, local Whisper CLI ASR, local ffmpeg keyframes. |
| v1.5 | Template packs | Exam, teacher, developer, creator, and enterprise-training packs. |
| v1.6+ | Experimental adapters | Explicitly enabled, low-frequency, user-authorized adapters with no cookie replay, paywall bypass, DRM bypass, anti-bot bypass, or signature bypass. |
...
| DOCX / Word | Implemented OpenXML extraction with source fidelity warnings. | User-uploaded Word extraction with headings, body text, tables, images, comments, footnotes, headers/footers. | Richer layout, tracked changes, and embedded-object extraction. | Silent table/image/comment loss, macro execution, hidden private content leakage. |
| PPTX / slides | Implemented OpenXML extraction with source fidelity warnings. | Slide text, image counts, chart counts, and speaker notes. | Richer chart data, image OCR, and animation/order modeling. | Silent speaker-note loss, chart data loss, animation/order confusion. |
...
| Local video/audio | Implemented for user-provided transcripts and optional local helper commands. | Uploaded `.srt`, `.vtt`, transcript `.txt`, local Whisper CLI ASR, and local ffmpeg interval keyframes. | Scene/slide-aware screenshots and aligned ASR evidence. | Default ASR, bulk media download, DRM bypass. |
